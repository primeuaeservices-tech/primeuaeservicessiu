...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'query_table': {
        const { table, select = '*', filter, orderBy, orderDirection, limit } = args as any;
      ...
      case 'create_user': {
        const { email, password, emailConfirmed = true } = args as any;
      ...
      case 'update_user_password': {
        const { userId, newPassword } = args as any;
